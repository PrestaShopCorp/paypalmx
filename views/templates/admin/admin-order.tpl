...
<fieldset>
	...
	<table cellpadding="0" cellspacing="0" class="table">
		...
		<tr>
			...
			<td>{if $paypal_mx_transaction_details.mode == 'test'}<span style="color: #CC0000;">{l s='Sandbox (Test)' mod='paypalmx'}</span>{else}{l s='Live' mod='paypalmx'}{/if}</td>
		</tr>
		<tr>
...
<br />
<fieldset>
	<legend><img src="{$module_dir}logo.gif" alt="" /> {l s='Proceed to a full or partial refund via PayPal' mod='paypalmx'}</legend>
	{if isset($paypal_mx_refund) && $paypal_mx_refund}
		<div class="conf">{l s='Refund successfully performed' mod='paypalmx'}</div><br />
	{else}
		{if isset($paypal_mx_refund) && !$paypal_mx_refund}
		<div class="error">{l s='An error occured during this refund' mod='paypalmx'}{if isset($paypal_mx_refund_error) && $paypal_mx_refund_error} - {$paypal_mx_refund_error|escape:'htmlall':'UTF-8'}{/if}</div><br />
		{/if}
	{/if}
	{if $paypal_mx_more60d}
		<div class="info">{l s='This order has been placed more than 60 days ago or no transaction details are available. Therefore, it cannot be refunded anymore.' mod='paypalmx'}</div>
	{/if}
	...
	{else}
		<form method="post" action="" name="refund">
			{l s='Refund:' mod='paypalmx'} <input type="text" name="refund_amount" value="{($paypal_mx_transaction_details.amount-$total_refund)|floatval}" />
			<input type="hidden" name="id_transaction" value="{$paypal_mx_transaction_details.id_transaction|escape:'htmlall':'UTF-8'}" />
			<input type="submit" name="process_refund" value ="{l s='Process Refund' mod='paypalmx'}" class="button" />
		</form>
	{/if}
</fieldset>